...
$(".next").click(function () {
  if (!isUploadFile && $("fieldset").index($(this).parent()) == 0) {
...
const SCORE_INVALID_ASM_RANGE = [-1];
const SCORE_ERROR_ASM_RANGE = [-2];
...
// Handle file when uploaded
$("input[type=file]").on("change", function () {
  // Check number of uploaded file
  if ($("input[type=file]")[0].files.length > 2) {
    alert("You can select only 2 excel files");
  } else {
    if ($("input[type=file]")[0].files.length < 2) {
      alert("You must select 2 excel files");
    } else {
      let button = $(this);
...
function readFile(file) {
  ...
  reader.onload = function (e) {
    ...
    jsonData.forEach(function (object) {
      // distinguish CMS and FAP files and add data to corresponding array
      //function to handle data
      if (col_count != MAX_COLUMNS_NUMBER) {
...
function getAverageScore(score_range, asm2_score) {
  if (asm2_score > score_range[1]) {
    return (score_range[0] + score_range[1]) / 2;
  }
  return (score_range[0] + asm2_score) / 2;
}

// Function to handle score of asm 1 and asm 2
// Return the score by number
function handleScore(data) {
  // Get data by key as header of columns in CMS data
  let asm1_1st_score_real = data[cms_header_arr[CMS_SCORE_COLUMNS[0]]];
  let asm1_2nd_score_real = data[cms_header_arr[CMS_SCORE_COLUMNS[1]]];
  let asm2_1st_score_real = data[cms_header_arr[CMS_SCORE_COLUMNS[2]]];
  let asm2_2nd_score_real = data[cms_header_arr[CMS_SCORE_COLUMNS[3]]];
  // Check score
  let asm1_final = checkAssignmentScore(
    asm1_1st_score_real,
    asm1_2nd_score_real
  );
  let asm2_final = checkAssignmentScore(
    asm2_1st_score_real,
    asm2_2nd_score_real
  );
  switch (asm1_final) {
    case SCORE_FAIL_OR_REFER: {
      if (asm2_final == INVALID_ASM) {
        return INVALID_ASM;
      }
      return SCORE_FAIL_OR_REFER;
    }
    case INVALID_ASM: {
      return INVALID_ASM;
    }
    ...
    case SCORE_DISTINCTION: {
      switch (asm2_final) {
        ...
        case SCORE_MERIT:
        case SCORE_DISTINCTION: {
          return Math.min(asm1_final, asm2_final);
        }
        default: {
          return ERROR_ASM;
        }
...
// Check score by word and convert to number
function checkScore(score) {
  switch (score) {
    ...
    case SCORE_DISTINCTION_W: {
      return SCORE_DISTINCTION;
    }
    default: {
      return SCORE_FAIL_OR_REFER;
    }
...
function checkScoreRange(score) {
  switch (score) {
    case SCORE_PASS_W: {
      return SCORE_PASS_RANGE;
    }
    case SCORE_MERIT_W: {
      return SCORE_MERIT_RANGE;
    }
    case SCORE_DISTINCTION_W: {
      return SCORE_DISTINCTION_RANGE;
    }
    default: {
      return SCORE_FAIL_OR_REFER_RANGE;
    }
  }
}

function checkAssignmentScoreRange(score_real_1st, score_real_2nd) {
  console.log("Điểm chữ 1st chance: " + score_real_1st.toLowerCase().trim());
  let asm_final = checkScoreRange(score_real_1st.toLowerCase().trim());
  console.log(asm_final);
  if (asm_final[0] == SCORE_FAIL_OR_REFER) {
    console.log("Điểm chữ 2nd chance: " + score_real_2nd.toLowerCase().trim());
    asm_final = checkScoreRange(score_real_2nd.toLowerCase().trim());
    console.log(asm_final);
    if (asm_final[0] > SCORE_PASS) {
      return SCORE_INVALID_ASM_RANGE;
    }
  } else if (score_real_2nd.toLowerCase().trim() != SCORE_ERROR) {
    console.log("Check");
    // Check if asm1 valid but lecturer still give mark for 2nd chance 
    return SCORE_ERROR_ASM_RANGE;
  }
  console.log("Valid asm");
  return asm_final;
}

function getAverageScore(asm1_score_range, asm2_score_range) {
  if (asm2_score_range[0] > asm1_score_range[0]) {
    return (asm1_score_range[0] + asm1_score_range[1])/2;
  } else if (asm2_score_range[0] > asm1_score_range[0]) {
    return (asm2_score_range[0] + asm2_score_range[1])/2;
  } else {
    return asm1_score_range[0];
  }
}

function handleScore1(data) {
  // Get data by key as header of columns in CMS data
  let asm1_1st_score_real = data[cms_header_arr[CMS_SCORE_COLUMNS[0]]];
  let asm1_2nd_score_real = data[cms_header_arr[CMS_SCORE_COLUMNS[1]]];
  let asm2_1st_score_real = data[cms_header_arr[CMS_SCORE_COLUMNS[2]]];
  let asm2_2nd_score_real = data[cms_header_arr[CMS_SCORE_COLUMNS[3]]];
  // Check score
  let asm1_final = checkAssignmentScoreRange(
    asm1_1st_score_real,
    asm1_2nd_score_real
  );
  console.log("ASM1 Final: " + asm1_final);
  let asm2_final = checkAssignmentScoreRange(
    asm2_1st_score_real,
    asm2_2nd_score_real
  );
  console.log("ASM2 Final: " + asm2_final);
  switch (asm1_final[0]) {
    case SCORE_FAIL_OR_REFER: {
      if (asm2_final == INVALID_ASM) {
        return INVALID_ASM;
    ...
    case SCORE_DISTINCTION: {
      switch (asm2_final[0]) {
        ...
        case SCORE_MERIT:
        case SCORE_DISTINCTION: {
          return getAverageScore(asm1_final, asm2_final);
        }
        default: {
          return ERROR_ASM;
        }
...
// Function to handle couple of score for each asm (1 and 2)
function checkAssignmentScore(score_real_1st, score_real_2nd) {
  let asm_final = checkScore(score_real_1st.toLowerCase().trim());
  if (asm_final == SCORE_FAIL_OR_REFER) {
    asm_final = checkScore(score_real_2nd.toLowerCase().trim());
    if (asm_final > SCORE_PASS) {
      return INVALID_ASM;
    }
  } else if (score_real_2nd.toLowerCase().trim() != SCORE_ERROR) {
    return ERROR_ASM;
